...
from .shape_opt.results_ui import Ui_Form
from .shape_opt.path import p
# from .shape_opt.utils.ImageView import ImageView
...
class Results(Q.QWidget):
    ...
    def __init__(self, astergui, parent=None):
        ...
        self.sidebar.setupUi(left_container)
        self.sidebar.tabWidget.setCurrentIndex(0)
        # 拖动条
        ...
        self.sidebar.pushButton_23.clicked.connect(self.load_fem_results)
        connect(self.sidebar.comboBox_53.currentIndexChanged,self.changeFemComponent)
        ...
        connect(self.sidebar.pushButton_24.clicked,self.updateFemChange)
        # 窗口2“打开结果文件”按钮
        self.sidebar.pushButton_6.clicked.connect(self.load_opt_results)
        connect(self.sidebar.pushButton_5.clicked,self.updateOptChange)
        connect(self.sidebar.comboBox.currentIndexChanged,self.Change_opt_results_type)
        connect(self.sidebar.comboBox_17.currentIndexChanged,self.changeOptFemComponent)

        connect(self.sidebar.tabWidget.currentChanged,self.changeview)
        """
        ...
        with open(p.user_settings, 'r') as f:
            for line in f.readlines():
                ...
                if(line.find('self.working_dir') == 0):
                    ...
                    self.opt_path = os.path.join(self.current_working_dir,'opt')

    def read_opt_log(self):
        self.read_config()
        log_file_name = os.path.basename(self.model)
        log_base_path = os.path.join(self.current_working_dir,'opt')
        log_path = os.path.join(log_base_path,log_file_name) + '.log'
        iter_num = []
        with open(log_path, 'r') as f:
            for line in f.readlines():
                if(line.find('iter_num:') == 0):
                    iter_num.append(line.split()[1])
        
        max_iter_num = int(iter_num[-1]) 
    ...
    def load_opt_results(self):
        try:
            self.sidebar.pushButton_6.setEnabled(False)
            self.sidebar.pushButton_5.setEnabled(False)
            self.read_config()
            iter_num = str(self.read_opt_log())
            # update iter_num in front
            self.sidebar.label.setText("总迭代次数: " + iter_num)
            log_base_path = os.path.join(self.current_working_dir,'opt')
            cmd = 'sh ' + p.ccx2paraview_run + " " + p.ccx2paraviewOpt + " " + iter_num + " " + log_base_path
            print("load_opt_results cmd:",cmd)
            self.process_opt.start(cmd)
            QtWidgets.QMessageBox.information(self, '提示', '正在转换结果，请稍等!')
            self.process_opt.waitForFinished(5000)
            QtWidgets.QMessageBox.information(self, '提示', '结果已经准备好!')
            self.sidebar.pushButton_6.setEnabled(True)
            self.sidebar.pushButton_5.setEnabled(True)
        except:
            self.sidebar.pushButton_6.setEnabled(True)
            self.sidebar.pushButton_5.setEnabled(True)
        ...
        try:
            ...
            #pvs.Delete(self.currentfilename)
            del self.currentdisplay
            self.opt_display.SetScalarBarVisibility(self.ren_view1, False)
            self.optWarpByVectorDisplay.SetScalarBarVisibility(self.ren_view1, False)

        except Exception as e:
            print(e)

        self.iter_list = [str(i) for i in range(1,int(iter_num)+1)]
        self.sidebar.comboBox_2.clear()
        self.sidebar.comboBox_2.addItems(self.iter_list)
        self.sidebar.comboBox_2.setCurrentIndex(int(self.iter_list[-1])-1)
        self.sidebar.comboBox_17.clear()
        self.sidebar.comboBox_17.addItem("SENSTRE")
        self.file_1 = [os.path.join(self.opt_path, "file" + str(i).zfill(3) + '.1.vtu') for i in range(1,int(iter_num) + 1)]
        self.file_2 = [os.path.join(self.opt_path, "file" + str(i).zfill(3) + '.2.vtu') for i in range(1,int(iter_num) + 1)]

        opt_res = pvs.XMLUnstructuredGridReader(FileName=self.file_1[-1])
        self.opt_display = pvs.Show(opt_res, self.ren_view1)
        self.opt_display.Representation = 'Surface'
        self.ren_view1.ResetCamera()
        self.ren_view1.Update()

        pvs.ColorBy(self.opt_display, ('POINTS', 'SENSTRE', 'Magnitude'))

        self.opt_display.RescaleTransferFunctionToDataRange(True, False)
        self.opt_display.SetScalarBarVisibility(self.ren_view1, True)
        sENSTRELUT = pvs.GetColorTransferFunction('SENSTRE')
        pvs.Render()
        self.currentdisplay = self.opt_display

    def updateOptChange(self):
        try:
            # 清除当前显示
            pvs.HideAll(self.ren_view1)
            pvs.Delete(self.currentdisplay)
        ...
        if self.sidebar.comboBox.currentIndex() == 1:

            fdir = self.file_2[int(self.sidebar.comboBox_2.currentIndex())]
            opt_res = pvs.XMLUnstructuredGridReader(FileName=fdir)
            self.opt_display = pvs.Show(opt_res, self.ren_view1)
            self.opt_display.Representation = 'Surface'
            ...
            if (self.sidebar.comboBox_17.currentIndex() == 0):
                pvs.ColorBy(self.opt_display, ('POINTS', 'U',self.sidebar.comboBox_10.currentText()))
                uLUT = pvs.GetColorTransferFunction('U')
                if self.sidebar.comboBox_18.currentIndex()==0:
                    uLUT.ColorSpace = 'Diverging'
                else:
                    uLUT.ColorSpace = 'RGB'

            if (self.sidebar.comboBox_17.currentIndex() == 1):
                pvs.ColorBy(self.opt_display, ('POINTS', 'S_Mises','S_Mises'))
                s_MisesLUT = pvs.GetColorTransferFunction('S_Mises')
                if self.sidebar.comboBox_18.currentIndex()==0:
                    s_MisesLUT.ColorSpace = 'Diverging'
                else:
                    s_MisesLUT.ColorSpace = 'RGB'

            array_surface = ['Surface','Surface With Edges','Points']
            self.opt_display.Representation = array_surface[self.sidebar.comboBox_9.currentIndex()]

            self.opt_display.RescaleTransferFunctionToDataRange(True, False)
            self.opt_display.SetScalarBarVisibility(self.ren_view1, True)
            self.currentdisplay = self.opt_display
            # self.warpByVector = pvs.WarpByVector(Input=opt_res)
            # pvs.SetActiveSource(self.warpByVector)
            # self.optWarpByVectorDisplay = pvs.Show(self.warpByVector, self.ren_view1)
            # array_surface = ['Surface','Surface With Edges','Points']
            # self.optWarpByVectorDisplay.Representation = array_surface[self.sidebar.comboBox_9.currentIndex()]

            # self.optWarpByVectorDisplay.SetScalarBarVisibility(self.ren_view1, True)
            # pvs.Hide(self.opt_display, self.ren_view1)
            # self.warpByVector.Vectors = ['POINTS', 'U']
            # self.ren_view1.Update()
            # self.warpByVector.ScaleFactor = float(self.sidebar.lineEdit_3.text())
            # self.ren_view1.Update()
            # self.currentdisplay = self.warpByVector
        else:
              
            fdir = self.file_1[int(self.sidebar.comboBox_2.currentIndex())]
            opt_res_1 = pvs.XMLUnstructuredGridReader(FileName=fdir)
            self.opt_display_2 = pvs.Show(opt_res_1, self.ren_view1)
            self.opt_display_2.Representation = 'Surface'
            self.ren_view1.Update()
            pvs.ColorBy(self.opt_display_2, ('POINTS', 'SENSTRE', 'Magnitude'))
            self.opt_display_2.RescaleTransferFunctionToDataRange(True, False)
            self.opt_display_2.SetScalarBarVisibility(self.ren_view1, True)
            array_surface = ['Surface','Surface With Edges','Points']
            self.opt_display_2.Representation = array_surface[self.sidebar.comboBox_9.currentIndex()]
            sENSTRELUT = pvs.GetColorTransferFunction('SENSTRE')
            if self.sidebar.comboBox_18.currentIndex()==0:
                sENSTRELUT.ColorSpace = 'Diverging'
            else:
                sENSTRELUT.ColorSpace = 'RGB'
            self.currentdisplay = self.opt_display_2
        pvs.Render()

        
    def Change_opt_results_type(self):
        if self.sidebar.comboBox.currentIndex() == 1:
            array_1 = ['Magnitude','D1','D2','D3']
            self.sidebar.comboBox_10.clear()
            self.sidebar.comboBox_10.addItems(array_1)

            array_2 = ['位移','Miss应力']
            self.sidebar.comboBox_17.clear()
            self.sidebar.comboBox_17.addItems(array_2)
            self.sidebar.comboBox_2.setCurrentIndex(int(self.iter_list[-1])-1)
    ...
    def changeOptFemComponent(self):
        if self.sidebar.comboBox_17.currentIndex() == 0 and self.sidebar.comboBox.currentIndex() == 1:
            array_1 = ['Magnitude','D1','D2','D3']
            self.sidebar.comboBox_10.clear()
            self.sidebar.comboBox_10.addItems(array_1)
        else:
    ...
    def load_fem_results(self):
        ...
        self.process_fem.waitForFinished(1000)
        ...
        try:
            ...
            print(e)
        fdir = self.model + '.2.vtu'
        self.static = pvs.XMLUnstructuredGridReader(FileName=fdir)
        ...
        uLUT = pvs.GetColorTransferFunction('U')
        pvs.ColorBy(self.staticDisplay, ('POINTS', 'U', 'Magnitude'))
        ...
        pvs.UpdateScalarBarsComponentTitle(uLUT, self.staticDisplay)
    ...
    def updateFemChange(self):
        ...
        #vector_array = ['Magnitude','D1','D2','D3']
        if (self.sidebar.comboBox_53.currentIndex() == 0):
            pvs.ColorBy(self.static_resrmedDisplay, ('POINTS', 'U',self.sidebar.comboBox_54.currentText()))
            uLUT = pvs.GetColorTransferFunction('U')
        ...
        if (self.sidebar.comboBox_53.currentIndex() == 1):
            s_MisesLUT = pvs.GetColorTransferFunction('S_Mises')
            pvs.ColorBy(self.static_resrmedDisplay, ('POINTS', 'S_Mises','S_Mises'))
            if self.sidebar.comboBox_56.currentIndex()==0:
                s_MisesLUT.ColorSpace = 'Diverging'
            else:
                s_MisesLUT.ColorSpace = 'RGB'
        ...
        self.warpByVector1.Vectors = ['POINTS', 'U']
        self.ren_view.Update()
    ...
    def changeview(self):
        i = self.sidebar.tabWidget.currentIndex()
        print(i)
        if i == 0:
            pvs.SetActiveView(self.ren_view)
        elif i == 1:
    ...
    def update_pv_layout_view(self, full_update=True):
        ...
        if not pv_layout:
            ...
            if self.windows==2: #两个窗口，则对半分割;可在此处修改窗口数量为1，即(0,1)
