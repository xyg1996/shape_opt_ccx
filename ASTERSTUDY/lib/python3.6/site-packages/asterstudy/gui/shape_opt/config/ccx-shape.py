...
max_node_shift = float(sys.argv[7])  # maximal node shift during one iteration in length units
...
iterations_max = int(sys.argv[5])  # maximum number of design iterations
convergence_tolerance = float(sys.argv[6])  # stop iteration if change in objectives is below this value, None - not to use it
...
def clear_log(file_dir):
    log_file = file_dir[:-4] + ".log"
...
def plot_result(file_dir,iter_num,volume_list,objectives_list,path):
    ...
    # # host.set_ylim(0, 120)
    # # par1.set_ylim(float(objectives_list[0])*0.5, float(objectives_list[0]))
    ...
    # plt.plot(iter_list, objectives_list, 'o-', label="Objective Function")
    # axes = plt.gca()
    # axes.set_xlim(0, int(i))
    # axes.set_ylim([float(objectives_list[0])*0.5, float(objectives_list[0])])
    # # plot2 = plt.plot(X,Y,'o',color='b')#,label='point 1-data')
...
# argv in command line
num_pro = sys.argv[1]
cwd = sys.argv[2]
com_queue = sys.argv[3]
model_inp = sys.argv[4]

path = os.path.join(cwd,'opt')
# make opt path
mkdir(path)
file_name = os.path.basename(str(model_inp)) + '.inp'
file_dir = os.path.join(path,file_name) # initial file name is the same as inp file
if not os.path.exists(file_dir):
    cmd_cp = 'cp' + ' ' + str(model_inp) + '.inp' + ' ' + str(file_dir)
    os.system(cmd_cp)
clear_log(file_dir)
...
msg = "\n"
msg += "---------------------------------------------------\n"
msg += "num_pro = " + str(num_pro) + "\n"
msg += "cwd = " + str(cwd) + "\n"
msg += "com_queue = " + str(com_queue) + "\n"
msg += "model_inp = " + str(model_inp) + "\n"
msg += "max_iter_number = " + str(iterations_max) + "\n"
msg += "max_node_shift = " + str(max_node_shift) + "\n"
write_to_log(file_dir, msg)
...
os.putenv('OMP_NUM_THREADS', str(num_pro))
...
while True:
    ...
    # cmd = '/opt/skyformai/bin/csub -I -cwd ' + str(cwd) 
    # cmd = cmd + ' ' + '-q' + ' ' + str(com_queue) + ' ' + '-n' + ' ' + str(num_pro) + ' '
    # subprocess.call(cmd + os.path.normpath(path_calculix) + " " + file_i, shell=True, cwd=path)
    ...
    if i > iterations_max:
        break
    if i != 1 and convergence_tolerance:
        converged = []
        for obj in objectives:
    ...
    msg = "\n"
    msg += ("iter_num: " + str(i-1))
    write_to_log(file_dir,msg)
    plot_result(file_dir,i,volume_list,objectives_list,path)
