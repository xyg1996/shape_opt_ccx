...
from PyQt5.QtWidgets import QApplication, QDialog,QFrame,QProgressDialog
...
from path import p # change_dir,so the current direct is shape_opt
...
import log
from ..utils.userfulFun import read_config
...
class Main(QtWidgets.QMainWindow):
    ...
    def __init__(self,input,xml=p.main_xml):
        ...
        try:
            if read_config() != False:
                self.start_model,self.current_working_dir = read_config()
                self.j = Job()
                if self.start_model + '.inp' != p.default_inp and self.start_model is not None:
                    # try:
                        self.read_thread = MyThread_create_tree_view(self.model,self.start_model,self.j)
                        #self.import_file(self.start_model + '.inp')
                        connect(self.meshFileChanged, self.meshview.displayMEDFileName)
                        #self.open_file(start_model + '.med')
                        self.read_thread.start()
                        #self.j = self.read_thread.j
                    # except:
                    #     pass
        ...
        self.action_job_export_vtu.triggered.connect(lambda: self.open_paraview())
        #self.show()
        # self.ui = Ui_Form()
    ...
    def close_progress_bar(self):
        self.my_thread_process.terminate()	# 结束此进程
        self.my_thread_process.wait()		# 等待结束完成
        if self.my_thread_process.isFinished():	# 如果当前线程已经完成工作，则删除
            self.my_thread_process.exit
            del self.my_thread_process
            self.progress.setValue(100)
            QtWidgets.QMessageBox.information(self,"提示","导入成功")

    def set_progress_bar_value(self, num):  # 4
    ...
    def import_file(self, file_name):
        ...
        if file_name is None:
            ...
            try:
                ...
                file_name = QtWidgets.QFileDialog.getOpenFileName(self, \
                    'Import INP/FBD/UNV file', self.current_working_dir, \
                    'INP (*.inp);;FBD (*.fbd);;UNV (*.unv)')[0]
            except:
                file_name = QtWidgets.QFileDialog.getOpenFileName(self, \
            ...
            #QtWidgets.QMessageBox.information(self, '提示', '正在导入模型数据，请稍等!')
            ...
            # self.progress.setLabelText("正在导入模型数据...")
            # self.progress.setCancelButtonText("取消")
            # self.progress.setMinimumDuration(5)
            # self.progress.setWindowModality(Qt.WindowModal)
            # self.progress.setRange(0,100)
            # self.my_thread_process = MyThread_processbar()
            # self.my_thread_process.my_signal.connect(self.set_progress_bar_value)
            # self.finish_signal.connect(self.close_progress_bar)
            # self.my_thread_process.start()
            # try: 
            self.generateTreeView()
            # except:
    ...
    def write_input(self, lines):
        ...
        try:
            file_name = QtWidgets.QFileDialog.getSaveFileName(None, \
                'Write INP file', self.current_working_dir, \
                'Input files (*.inp)')[0]
            if len(file_name):
            ...
            QtWidgets.QMessageBox.information(self, '提示', '请先导入INP文件!')
    ...
    def open_paraview(self):
        try:
            command = 'vglrun ' + p.path_paraview
...
# Prepare to import model
    ...
    def clicked(self):
        ...
        if item and item.itype == ItemType.GROUP:
            print("GROUP:",item.name)
            try:
                if item.name == "Mesh":
                    read_config()
                    self.file_name, self.nom_med = self.get_cmd_mesh(self.start_model + '.med')
                    print("self.file_name:",self.file_name)
                    print("self.nom_med:",self.nom_med)
    ...
    def startmesh(self,fname):
        ...
        self.file_name, self.nom_med = self.get_cmd_mesh(fname)
...
class MyThread_processbar(QThread):
    my_signal = pyqtSignal(int)

    def __init__(self):
        super(MyThread_processbar, self).__init__()
...
class MyThread_create_tree_view(QThread):
    my_signal = pyqtSignal(int)

    def __init__(self,model,start_model,job):
        super(MyThread_create_tree_view, self).__init__()
        self.model = model
