...
class Settings:
    ...
    def __init__(self):
        ...
        try:
            ...
            with open(p.user_settings, mode='w', encoding='utf-8') as ff:
                print("用户配置文件创建成功！")
            if os.name=='nt':
                # Windows
                ext = '.exe'
                ...
                self.path_paraview = p.user_homedir
                self.working_dir = os.path.join(p.user_homedir,'ccxShapeOpt')
                self.start_model = os.path.join(self.working_dir, 'connecting_rod-ccx.inp')
    ...
    def save(self):
        ...
        sd = SettingsDialog(settings=self)
        sd.save()
        # self.current_working_dir = sd.dir_name
        # print('self.current_working_dir:',self.current_working_dir)
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, settings=None):
        ...
        self.path_paraview_button.clicked.connect(
            lambda: self.select_dir(self.working_dir))
    ...
    def save(self):
        ...
        with open(p.user_settings, 'w') as f:
    ...
    def select_path(self, path_edit):
        ...
        file_name = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Select path', '', 'INP (*.inp)')[0]
        if len(file_name):
            path_edit.setText(file_name)
    
    def select_dir(self, path_edit):
        """Open file dialog to select path."""
        dir_name = QtWidgets.QFileDialog.getExistingDirectory(
            self, 'Select dir',p.user_homedir,QtWidgets.QFileDialog.ShowDirsOnly)
        if len(dir_name):
            path_edit.setText(dir_name)
