...
from .path import p
from .utils.ImageView import ImageView
...
class Maincontrol(Q.QWidget):
    def __init__(self,qianchuli):
        ...
        #self.ui.pushButton_3.clicked.connect(self.open_image)
        self.ui.pushButton_4.clicked.connect(self.kill_opt)
        self.ui.pushButton_4.clicked.connect(self.open_image)
    ...
    def kill_fem(self):
        ...
        # if self.process_fem.ProcessState() == 0:
        #     QtWidgets.QMessageBox.information(self, '提示', '您没有正在运行的仿真计算作业!')
        # else:
        try:
    ...
    def start_opt(self):
        ...
        try:
            ...
            self.node_shift_max,self.iterations_max,convergence_tolerance = self.opt_dialog()
            ...
            if self.iterations_max and convergence_tolerance:
                ...
                cmd = cmd + " " + str(self.num_pro) + " " + self.current_working_dir
                cmd = cmd + " " + str(self.queue) + " " + self.model
                cmd = cmd + " " + str(self.iterations_max) + " " + str(convergence_tolerance) + " " + str(self.node_shift_max)
    ...
    def kill_opt(self):
        # if self.process_opt.ProcessState() == 0:
        #     QtWidgets.QMessageBox.information(self, '提示', '您没有正在运行的优化作业!')
        # else:
        try:
    ...
    def opt_dialog(self):
        ...
        max_node_shift = QLineEdit()
        max_node_shift.setText('0.3')
        formLayout.addRow('最大节点移动', max_node_shift)
        ...
        if dialog.exec() == QDialog.Accepted:
            return max_node_shift.text(),iterations_max.text(), convergence_tolerance.text()
        else:
            return 0,0

    def open_image(self):
        image_path = os.path.join(self.current_working_dir,'opt')
        image_path = os.path.join(image_path,'results.png')
        if os.path.exists(image_path):
            self.image = ImageView(image=image_path, background=Qt.black)
            self.image.show()
